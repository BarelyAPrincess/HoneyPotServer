...
import io.amelia.support.LibMath;
 ...
 * should work directly with either the {@link BaseBundle} or
...
public class BaseBundle
{
	...
	private static final Logger L = LogBuilder.get( BaseBundle.class );
	 ...
	 * @param capacity Initial size of the HashMap.
	 */
	...
	{
		unparcel();
		L.debug( "Getting boolean in " + Integer.toHexString( System.identityHashCode( this ) ) );
	...
	public String getPairValue()
	{
		...
		if ( size > 1 )
		{
			L.warning( "getPairValue() used on Bundle with multiple pairs." );
		}
		if ( size == 0 )
		{
			return null;
		}
		Object o = Maps.first( mMap );
		try
		{
			return ( String ) o;
		}
	...
	{
		return mParcelledData == EMPTY_PARCEL;
	}
	...
	private void readFromParcelInner( Parcel parcel, int length )
	{
		...
		parcel.setDataPosition( LibMath.addOrThrow( offset, length ) );

		Parcel p = Parcel.obtain();
		p.setDataPosition( 0 );
		p.appendFrom( parcel, offset, length );
		L.debug( "Retrieving " + Integer.toHexString( System.identityHashCode( this ) ) + ": " + length + " bundle bytes starting at " + offset );
		p.setDataPosition( 0 );
	...
	void typeWarning( String key, Object value, String className, Object defaultValue, ClassCastException e )
	{
		...
		L.warning( "Attempt to cast generated internal exception:", e );
	}
	...
	synchronized void unparcel()
	{
		synchronized ( this )
		{
			...
			{
				L.debug( "unparcel " + Integer.toHexString( System.identityHashCode( this ) ) + ": no parcelled data" );
				return;
			}

			if ( LOG_DEFUSABLE && sShouldDefuse && ( mFlags & FLAG_DEFUSABLE ) == 0 )
				L.severe( "Attempting to unparcel a Bundle while in transit; this may clobber all data inside!", new Throwable() );

			if ( isEmptyParcel() )
			{
				L.debug( "unparcel " + Integer.toHexString( System.identityHashCode( this ) ) + ": empty" );
				if ( mMap == null )
					mMap = new ArrayMap<>( 1 );
				else
					mMap.clear();
			...
			L.debug( "unparcel " + Integer.toHexString( System.identityHashCode( this ) ) + ": reading " + N + " maps" );
			...
			try
			{
				parcelledData.readHashMapInternal( map, N, mClassLoader );
			}
			catch ( BadParcelableException e )
			{
				if ( sShouldDefuse )
				{
					L.warning( "Failed to parse Bundle, but defusing quietly", e );
					map.erase();
				}
				else
				{
			...
			L.debug( "unparcel " + Integer.toHexString( System.identityHashCode( this ) ) + " final map: " + mMap );
		}
	}
	...
	void writeToParcelInner( Parcel parcel, int flags )
	{
		...
		else
		{
			...
			parcel.writeHashMapInternal( mMap );
