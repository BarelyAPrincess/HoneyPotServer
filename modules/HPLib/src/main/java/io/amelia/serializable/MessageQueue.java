...
public final class MessageQueue
{
	...
	private HashMap<Integer, FileDescriptorRecord> mFileDescriptorRecords;
	 ...
	 */
	public void addOnFileDescriptorEventListener( @NotNull FileDescriptor fd, @OnFileDescriptorEventListener.Events int events, @NotNull OnFileDescriptorEventListener listener )
	{
		if ( fd == null )
		{
	...
	private int dispatchEvents( int index, int events )
	{
		...
		synchronized ( this )
		{
			record = mFileDescriptorRecords.get( index );
			if ( record == null )
			{
		...
		if ( newWatchedEvents != oldWatchedEvents )
		{
			synchronized ( this )
			{
				if ( index >= 0 && mFileDescriptorRecords.get( index ) == record && record.mSeq == seq )
				{
					record.mEvents = newWatchedEvents;
					if ( newWatchedEvents == 0 )
					{
						mFileDescriptorRecords.remove( index );
					}
				}
			}
		}
	 ...
	 */
	public void removeOnFileDescriptorEventListener( @NotNull FileDescriptor fd )
	{
		if ( fd == null )
		{
			throw new IllegalArgumentException( "fd must not be null" );
	...
	private void updateOnFileDescriptorEventListenerLocked( FileDescriptor fd, int events, OnFileDescriptorEventListener listener )
	{
		...
		if ( events != 0 )
		{
			...
			{
				if ( mFileDescriptorRecords == null )
					mFileDescriptorRecords = new HashMap<>();
	...
	public interface OnFileDescriptorEventListener
	{
		 ...
		 */
		@Events
		int onFileDescriptorEvents( @NotNull FileDescriptor fd, @Events int events );
