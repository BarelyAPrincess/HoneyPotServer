...
public final class ArrayMap<K, V> implements Map<K, V>
{
	...
	private static final Logger L = LogBuilder.get( ArrayMap.class );
		...
		if ( hashes.length == ( BASE_SIZE * 2 ) )
		{
			synchronized ( ArrayMap.class )
			{
				if ( mTwiceBaseCacheSize < CACHE_SIZE )
				{
					...
					mTwiceBaseCacheSize++;
					L.debug( "Storing 2x cache " + array + " now have " + mTwiceBaseCacheSize + " entries" );
				}
			}
		}
		else if ( hashes.length == BASE_SIZE )
		{
			synchronized ( ArrayMap.class )
			{
				if ( mBaseCacheSize < CACHE_SIZE )
				{
					...
					mBaseCache = array;
					mBaseCacheSize++;
					L.debug( "Storing 1x cache " + array + " now have " + mBaseCacheSize + " entries" );
				}
			}
		}
	}
		...
		if ( size == ( BASE_SIZE * 2 ) )
		{
			synchronized ( ArrayMap.class )
			{
				if ( mTwiceBaseCache != null )
				{
					...
					mTwiceBaseCacheSize--;
					L.debug( "Retrieving 2x cache " + mHashes + " now have " + mTwiceBaseCacheSize + " entries" );
					return;
				}
			}
		}
		else if ( size == BASE_SIZE )
		{
			synchronized ( ArrayMap.class )
			{
				if ( mBaseCache != null )
				{
					...
					array[0] = array[1] = null;
					mBaseCacheSize--;
					L.debug( "Retrieving 1x cache " + mHashes + " now have " + mBaseCacheSize + " entries" );
					return;
				}
			}
		}
	...
	public void append( K key, V value )
	{
		...
		if ( index > 0 && mHashes[index - 1] > hash )
		{
			...
			e.fillInStackTrace();
			L.warning( "New hash " + hash + " is before end of array hash " + mHashes[index - 1] + " at index " + index + " key " + key, e );
			put( key, value );
			return;
		}
		mSize = index + 1;
	...
	{
		return Maps.containsAll( this, collection );
	}
	...
	int indexOf( Object key, int hash )
	{
		...
		int index = Arrs.binarySearch( mHashes, N, hash );
	...
	int indexOfNull()
	{
		...
		int index = Arrs.binarySearch( mHashes, N, 0 );
	...
	{
		return Maps.removeAll( this, collection );
	}
	...
	public V removeAt( int index )
	{
		...
		{
			// Now empty.
			L.debug( "remove: shrink from " + mHashes.length + " to 0" );
		...
		else
		{
			if ( mHashes.length > ( BASE_SIZE * 2 ) && mSize < mHashes.length / 3 )
			{
				...
				L.debug( "remove: shrink from " + mHashes.length + " to " + n );
				...
				if ( index > 0 )
				{
					L.debug( "remove: copy from 0-" + index + " to 0" );
					System.arraycopy( ohashes, 0, mHashes, 0, index );
					System.arraycopy( oarray, 0, mArray, 0, index << 1 );
				}
				if ( index < mSize )
				{
					L.debug( "remove: copy from " + ( index + 1 ) + "-" + mSize + " to " + index );
			...
			else
			{
				...
				if ( index < mSize )
				{
					L.debug( "remove: move " + ( index + 1 ) + "-" + mSize + " to " + index );
	...
	{
		return Maps.retainAll( this, collection );
	}
	...
	public V put( K key, V value )
	{
		...
		if ( mSize >= mHashes.length )
		{
			...
			L.debug( "put: grow from " + mHashes.length + " to " + n );
		...
		if ( index < mSize )
		{
			L.debug( "put: move " + index + "-" + ( mSize - index ) + " to " + ( index + 1 ) );
