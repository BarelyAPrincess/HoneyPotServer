...
public class GroupCommands extends PermissionBaseCommand
{
	...
	public void entitySwapPermission( TerminalEntity sender, Map<String, String> args )
	{
		...
		try
		{
			int sourceIndex = getPosition( autoCompletePermission( group, args.get( "io/io.amelia/permission" ), refs, "io/io.amelia/permission" ), permissions );
	...
	public void groupAddPermission( TerminalEntity sender, Map<String, String> args )
	{
		...
		try
		{
			group.addPermission( args.get( "io/io.amelia/permission" ), true, refs );
		}
		...
		sender.sendMessage( EnumColor.WHITE + "Permission \"" + args.get( "io/io.amelia/permission" ) + "\" added to " + group.getId() + " !" );
	...
	public void groupAddTimedPermission( TerminalEntity sender, Map<String, String> args )
	{
		...
		group.addTimedPermission( args.get( "io/io.amelia/permission" ), true, refs, lifetime );

		sender.sendMessage( EnumColor.WHITE + "Timed permission added!" );
		informGroup( group, "Your permissions have been changed!" );

		PermissionDispatcher.getLogger().info( "Group " + groupName + " get timed permission \"" + args.get( "io/io.amelia/permission" ) + "\" " + ( lifetime > 0 ? "for " + lifetime + " seconds " : " " ) + "from " + sender.getDisplayName() );
	}
	...
	public void groupRemovePermission( TerminalEntity sender, Map<String, String> args )
	{
		...
		String permission = autoCompletePermission( group, args.get( "io/io.amelia/permission" ), refs );
	...
	public void groupRemoveTimedPermission( TerminalEntity sender, Map<String, String> args )
	{
		...
		group.removeTimedPermission( args.get( "io/io.amelia/permission" ), refs );

		sender.sendMessage( EnumColor.WHITE + "Timed permission \"" + args.get( "io/io.amelia/permission" ) + "\" removed!" );
		informGroup( group, "Your permissions have been changed!" );
	}
