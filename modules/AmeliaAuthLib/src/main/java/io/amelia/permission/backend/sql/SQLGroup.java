	...
	public void reloadPermissions()
	{
		...
		try
		{
			...
			if ( rs.count() > 0 )
				do
				{
					PermissionNamespace ns = PermissionNamespace.parseString( rs.getString( "io/io.amelia/permission" ) );
	...
	public void save()
	{
		...
		try
		{
			...
			for ( ChildPermission child : children )
			{
				...
				// child.getObject() ) );
				db.table( "permissions_entity" ).insert().value( "owner", getId() ).value( "type", 1 ).value( "refs", child.getReferences().join() ).value( "io/io.amelia/permission", perm.getNamespace() ).value( "value", child.getObject() ).executeWithException();
			}
