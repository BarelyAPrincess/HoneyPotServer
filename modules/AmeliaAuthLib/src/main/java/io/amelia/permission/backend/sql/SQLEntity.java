	...
	public void reloadPermissions()
	{
		...
		try
		{
			...
			if ( select.count() > 0 )
				...
				{
					PermissionNamespace ns = PermissionNamespace.parseString( row.get( "io/io.amelia/permission" ) );
	...
	public void save()
	{
		...
		try
		{
			...
			for ( ChildPermission child : children )
			{
				...
				// child.getObject() ) );
				db.table( "permissions_entity" ).insert().value( "owner", getId() ).value( "type", 0 ).value( "refs", child.getReferences().join() ).value( "io/io.amelia/permission", perm.getNamespace() ).value( "value", child.getObject() ).executeWithException();
			}
